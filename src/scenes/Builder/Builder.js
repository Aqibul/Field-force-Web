...
export default function Builder() {
  ...
  const handleForm = () => {
    ...
    dispatch(addAllDealer(formDataWithDefaults))
      ...
      .catch((error) => {
        ...
        toast.error("Error adding Builder. Please try again.");
      });
  };
  ...
  const handleEdit = () => {
    ...
    dispatch(updateAllDealer(credentials))
      ...
      .catch((error) => {
        ...
        toast.error("Error updating Builder. Please try again.");
      });
  };
  ...
  const filteredDataWithRole = filteredData?.filter(
    (dealerData) => dealerData.roleId === "6"
  );
  ...
  const handleOpenMapModal = (dealerData) => {
    ...
    if (isNaN(latitude) || isNaN(longitude)) {
      ...
      toast.error("Latitude and/or Longitude not available for this Builder.");
      return;
    }
    ...
    <>
      ...
      <div style={{ padding: "20px" }}>
        ...
        >
          ...
          <h4 style={{ color: "#ffffff" }}>Builder Details</h4>
          <InputBase
            sx={{ ml: 2 }}
        ...
        <Dialog open={openEditModal} onClose={handleCloseModal}>
          <DialogTitle>Update Builder</DialogTitle>
          <DialogContent>
            <Grid container spacing={2}>
              <input
                type="hidden"
